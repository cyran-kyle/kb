...
console.log(color('\nHello, I am Cyran Kyle , the main Developer of this bot.\n\nThanks for using: Turbo Bot','aqua'))
console.log(color('\nYou can follow me on Insta: cyran_kyle 0071','aqua'))

    let { version, isLatest } = await fetchLatestBaileysVersion()
    const A17 = A17Connect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['Turbo Bot By: Cyran Kyle ','Safari','1.0.0'],
        auth: state,
        version
    })
...
A17.ev.on('groups.update', async pea => {
       ...
       A17.send5ButImg(pea[0].id, `Group has been *Closed!* Only *Admins* can send Messages!`, `${BotName}`, wm_fatih, [])
...
*/
    ...
    A17.ev.on('groups.update', async pea => {
     ...
     // Get Profile Picture Group
        ...
        let wm_fatih = { url : ppgc }
        if (pea[0].announce == true) {
        //A17.send5ButImg(pea[0].id, `Group has been *Closed!* Only *Admins* can send Messages!`, `A17 Bot`, wm_fatih, [])

        A17.sendMessage(m.chat, { image: wm_fatih, caption: 'Grop has been *Closed!* Only *Admins* can send Messages!'})
        } else if(pea[0].announce == false) {
       // A17.send5ButImg(pea[0].id, `Group has been *Opened!* Now *Everyone* can send Messages!`, `A17 Bot`, wm_fatih, [])
...
I am *Turbo Bot*, Welcome to ${metadata.subject}.
